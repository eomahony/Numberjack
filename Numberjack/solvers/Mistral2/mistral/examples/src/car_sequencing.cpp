...
public:
  ...
  PboFormat(CarSequencingInstance *inst, const bool r) : CarSequencingModel(inst), randomized(r) {}
  virtual ~PboFormat() {}
...
void PboFormat::setup() {
  ...
  int num_vars = N*(M+O);
  int * shuffled_vars = new int[num_vars];
  ...
  if(randomized) {
    for(int i=0; i<num_vars-1; ++i) {
      int j = i+randint(num_vars-i);
      int aux = shuffled_vars[i];
      shuffled_vars[i] = shuffled_vars[j];
      shuffled_vars[j] = aux;
    }
  }
  ...
  for(int pos=0; pos<N; ++pos) {
    for(int cla=0; cla<M; ++cla) {
      std::cout << " +1 x" << shuffled_vars[(pos*M+cla)];
    }
    std::cout << " = 1 ;\n";
  }
  ...
  for(int car=0; car<N; ++car)
      ...
      for(int cla=0; cla<M; ++cla)
	{
	  for(int opt=0; opt<O; ++opt)
		...
		{
		  std::cout << " -1 x" << shuffled_vars[(car*M+cla)] << " +1 x" << shuffled_vars[(N*M+car*O+opt)] << " >= 0 ;\n";
		}
	      else
		{
		  std::cout << " 1 x" << shuffled_vars[(car*M+cla)] << " +1 x" << shuffled_vars[(N*M+car*O+opt)] << " <= 1 ;\n";		  
		}
	    }
	}
    }
  ...
  for(int opt=0; opt<O; ++opt) {
    // demand on options as boolean sums too
    for(int car=0; car<N; ++car) {
      std::cout << " +1 x" << shuffled_vars[(N*M+car*O+opt)];
    }
    ...
    for(int i=0; i<n; ++i) {
      for(int j=0; j<q; ++j)
	std::cout << " +1 x" << shuffled_vars[(N*M+(i+j)*O+opt)];
      std::cout << " <= " << p << " ;\n";
    }
  }
  ...
  for(int cla=0; cla<M; ++cla) {
    // demand on classes
    for(int car=0; car<N; ++car) {
      std::cout << " +1 x" << shuffled_vars[(car*M+cla)];
    }
    std::cout << " = " << instance->nb_cars_of_class(cla) << " ;\n";
  }

  delete [] shuffled_vars;
}
...
CarSequencingModel *modelFactory(string model, CarSequencingInstance *instance, bool rewrite , int clauses_model) {
	...
	if(model == "pbo") {
	  solver = new PboFormat(instance, false);
	} else if(model == "pbor") {
	  solver = new PboFormat(instance, true);
	} else if(model == "sum") {
...
int main(int argc, char **argv)
{
	try
	{
		...
		if(modelArg.getValue() != "pbo" && modelArg.getValue() != "pbor") {
