...
#define OsiXXXSolverInterface OsiCbcSolverInterface
#else
...
public:
	 ...
	 */
	Osi_IntVar(const int lb, const int ub, const int ident) :
			Osi_Expression(lb, ub) {
		nbj_ident = ident;
	}
	 ...
	 */
	Osi_IntVar(OsiIntArray& vals, const int ident) :
			Osi_Expression(vals) {
		nbj_ident = ident;
	}
...
public:
	Osi_Gcc(OsiExpArray& vars, OsiIntArray& vals, OsiIntArray& lb_card,
			OsiIntArray& ub_card);
...
public:
	 ...
	 * offset
	 */
	Osi_Sum(OsiExpArray& vars, OsiIntArray& weights, const int offset = 0);

	/**
	 * Create a weighted sum expression object that is the weighted sum
	 * of the two expressions and the integer array weights with an offset
	 */
	Osi_Sum(Osi_Expression* arg1, Osi_Expression* arg2, OsiIntArray& weights,
			const int offset);
	 ...
	 */
	Osi_Sum(Osi_Expression* arg, OsiIntArray& weights, const int offset);
...
public:
	 ...
	 *
	 */
	Osi_binop(Osi_Expression *var1, Osi_Expression *var2);
...
public:
	...
	int obj_id; // nbj_id of objective expression
	int maximise; // is the objective to maximise?

	// maps expression to variable index
	std::map<Osi_Expression*, int> vars;
	std::map<int, int> rows;
	std::vector<Osi_Expression*> exprs;
	...
	void addVar(Osi_Expression* expr);
	...
	// solving methods
	int solve();
	int solveAndRestart(const int policy = 0, const unsigned int base = 32,
			const double factor = 1.3333333, const double decay = 0.0,
			const int reinit = -1);
	int startNewSearch();
	int getNextSolution();
	...
	void setHeuristic(const char* var_heuristic, const char* val_heuristic,
			const int rand);
	...
	void addNogood(OsiExpArray& vars, OsiIntArray& vals);
	void guide(OsiExpArray& vars, OsiIntArray& vals, OsiDoubleArray& probs);
	...
	void setLowerBounds(OsiExpArray& vars, OsiIntArray& vals);
	void setUpperBounds(OsiExpArray& vars, OsiIntArray& vals);
