...
/*
  ...
  Generate and solve a sequence of CSPs
  with increasing looseness and given 
...
int Steps, timelimit, ins, Var, Dom, Con, Ari, Sup, Base, pi, pl, 
  probing, sac, rdz, lds, si, Gac;
float ubL, lbL;
bool ulw, uip;
string Heu, Pol;
long Seed;
...
const int nia = 18;
const char* int_ident[nia] = {"-s", "-i", "-t", "-v", "-d", "-a", "-b", "-pi", "-pl", "-probe", "-ulw", "-uip", "-sac", "-rdz", "-lds", "-si", "-st", "-g"};
int int_param[nia];

const int nsa = 4;
const char* str_ident[nsa] = {"-heu", "-res", "-l", "-u"};
...
{  
  ...
  ins       = ( int_param[1 ]  != NOVAL ? int_param[1 ] : 10  );//10  );
  timelimit = ( int_param[2 ]  != NOVAL ? int_param[2 ] : -1  );
  Var       = ( int_param[3 ]  != NOVAL ? int_param[3 ] : 30  );//100 );
  Dom       = ( int_param[4 ]  != NOVAL ? int_param[4 ] : 5   );//20  );
  Ari       = ( int_param[5 ]  != NOVAL ? int_param[5 ] : 3   );
  Base      = ( int_param[6 ]  != NOVAL ? int_param[6 ] : 32  );
  pi        = ( int_param[7 ]  != NOVAL ? int_param[7 ] : 100 );
  pl        = ( int_param[8 ]  != NOVAL ? int_param[8 ] : 30  );
  probing   = ( int_param[9 ]  != NOVAL ? int_param[9 ] : 0   );
  ulw       = ( int_param[10]  != NOVAL ? int_param[10] : 0   );
  uip       = ( int_param[11]  != NOVAL ? int_param[11] : 0   );
  sac       = ( int_param[12]  != NOVAL ? int_param[12] : 0   );
  rdz       = ( int_param[13]  != NOVAL ? int_param[13] : 0   );
  lds       = ( int_param[14]  != NOVAL ? int_param[14] : 0   );
  si        = ( int_param[15]  != NOVAL ? int_param[15] : -1  );
  Steps     = ( int_param[16]  != NOVAL ? int_param[16] : 10  );
  Gac       = ( int_param[17]  != NOVAL ? int_param[17] : VALID );
  
  Heu       = ( strcmp(str_param[0],"nil") ? str_param[0] : "dom/wdeg" );
  Pol       = ( strcmp(str_param[1],"nil") ? str_param[1] : "no"       );		
  lbL       = ( strcmp(str_param[2],"nil") ? atof(str_param[2]) : 0.01 );
  ubL       = ( strcmp(str_param[3],"nil") ? atof(str_param[3]) : 0.5 );  


  cerr <<"\n\t\t<"<<Var<<" "<<Dom<<" "
       << Ari << ">" << endl;

  for( double looseness = lbL; looseness <= ubL; looseness += (ubL-lbL)/(double)(Steps) )
    {

      double Cartesian = 1;
      for(int i=0; i<Ari; ++i)
	Cartesian *= (double)Dom;

      Sup = Cartesian * looseness;
      Con = ( -.95 * Var * log2(Dom) ) / log2(looseness);

      cerr << "[" << looseness << ": " << Con << ", " << Sup << "]" << endl;
  
      URCSPGenerator gen(Seed, Var, Dom, Con, Ari, Sup);
	  ...
	  if( status == LIMITOUT )
	    {
	      ...
	      addHeuristic(s);
	      if( Policy != NOVAL ) 
		status = s.solve_and_restart(Policy, Base, 1.33);
	      else if( lds )
		status = s.ldSolve();
	      else
		status = s.solve();
	...
	if( status == SAT )
	  ...
	  s.printStatistics(cerr, (OUTCOME | 
				   TOTALTIME | 
				   SPEEDB |
				   SPEEDP |
				   PPGS |
	...
	gen.reInit(Var, Dom, Con, Ari, Sup);
      }

      cout << setw(8) << looseness << " " 
	   << setw(4) << ((double)sat/(double)solved) 
	   << setw(9) << (avg_nds/ins) << " NDS"
	   << setw(9) << ( avg_time ? (unsigned long int)((double)avg_bts/avg_time) : 0 ) << " BTS/s"
	   << setw(9) << ( avg_time ? (unsigned long int)((double)avg_pgs/avg_time) : 0 ) << " PPGS/s"
	   << setw(14) << (avg_pgs/ins) << " PPGS"
	   << setw(8) << (avg_time/ins) << " s" << endl;
    }
}
